...
fi = katarchive.get_archived_products('1360834610.h5')
cold = katfile.open(fi[0])
fi = katarchive.get_archived_products('1360835888.h5')
hot = katfile.open(fi[0])

for diode in ['coupler','pin']:
	for pol in ['h','v']:
		f_num = 1 if diode == 'pin' else 3
		col = 'b' if diode == 'pin' else 'g'
		line = ':' if pol == 'h' else '-'
		ant = hot.ants[0].name
...
#temp_nd = nd.temperature(center_freqs / 1e6)
		...
		for i in range(6):
		    ...
		    tsys = cold_spec / gain
		    plt.plot(freq[i],tsys,col+line,label='nd_'+diode)
		    ...
		    tsys = Tcold + Trx 
		    plt.plot(freq[i],tsys,'r'+line,label='Y method')
		    ...
		    tdiode = Trx_nd - Trx
		    plt.figure(f_num+1)
		    plt.plot(freq[i],tdiode,'b'+line)
		    plt.plot(freq[i],np.array(nd_temp[i]),'r'+line)

		plt.figure(1)
		plt.ylim(0,50)
		plt.ylabel('Tsys/[K]')
		plt.xlabel('Freq/[Hz]')
		plt.title(ant+': Tsys at: '+cold.start_time.local())
		plt.grid()
		plt.figure(f_num+1)
		plt.ylim(0,5 if diode == 'coupler' else 80)
		plt.ylabel('Tdiode/[K]')
		plt.xlabel('Freq/[Hz]')
		plt.title(ant+': Tdiode_'+diode+' at: '+cold.start_time.local())
		plt.grid()

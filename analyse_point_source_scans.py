...
def reduce_compscan_with_uncertainty(dataset, compscan_index=0, mc_iterations=1, batch=True, **kwargs):
    ...
    #Identify the RFI channels in the compscan dataset then remove them
    rfi_channels = scape.plots_canned.identify_rfi_channels(compscan_dataset)
    logger.debug('compScan RFI')
    logger.debug(rfi_channels)
    freqkeep = np.tile(True, len(compscan_dataset.freqs))
    freqkeep[rfi_channels] = False
    compscan_dataset = compscan_dataset.select(freqkeep=freqkeep,copy=True)
    #end end scan dataset RFI removal
    cal_dataset = extract_cal_dataset(dataset)
    #identify the RFI channels in the cal dataset then remove them
    rfi_channels = scape.plots_canned.identify_rfi_channels(cal_dataset)
    logger.debug('cal_scan RFi')
    logger.debug(rfi_channels)
    freqkeep = np.tile(True, len(cal_dataset.freqs))
    freqkeep[rfi_channels] = False
    cal_dataset = cal_dataset.select(freqkeep=freqkeep,copy=True)
    #end cal dataset RFI removal
